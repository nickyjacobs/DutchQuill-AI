...
# ---------------------------------------------------------------------------
# Metadata-opschoning (anti-detectie)
# ---------------------------------------------------------------------------

def _sanitize_docx_metadata(output_path: str) -> None:
    """Schoon docProps/app.xml op — verwijder python-docx fingerprints.

    python-docx slaat standaard een app.xml op met AppVersion 14.0000
    (Word 2011) en TotalTime 0. Turnitin Document Details toont deze
    velden. Deze functie herschrijft ze naar realistische waarden.
    """
    import shutil
    import tempfile
        ...
        with zipfile.ZipFile(output_path, 'r') as zin, \
            ...
            for item in zin.infolist():
                ...
                if item.filename == 'docProps/app.xml':
                    ...
                    # AppVersion: 14.0000 (Word 2011 template) → 16.0000 (Word 365/2021)
                    el = root.find(f'{{{NS}}}AppVersion')
                    if el is not None:
                        el.text = '16.0000'
                    # TotalTime: 0 → realistisch (15-90 minuten)
                    el = root.find(f'{{{NS}}}TotalTime')
                    if el is not None:
                        el.text = str(random.randint(15, 90))
...
def build_document(data: dict, output_path: str) -> str:
    ...
    # Document-metadata opschonen (verwijder python-docx fingerprints)
    ...
    props.comments = ""           # Verwijder "generated by python-docx"
    props.revision = random.randint(8, 25)  # Variabel revisienummer
    now = datetime.now(timezone.utc)
    props.created = now - timedelta(hours=random.randint(2, 48))
    props.modified = now - timedelta(minutes=random.randint(5, 30))
    ...
    # Opslaan + metadata-opschoning (app.xml)
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    doc.save(output_path)
    _sanitize_docx_metadata(output_path)
